...
function AddForm({ members, onAdd }) {
  ...
  return <>
    ...
    <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginBottom: 16 }}>
      {CATS.map(c => <Chip key={c.id} active={f.cat === c.id} color={c.color} onClick={() => setF(x => ({...x, cat: c.id}))}>{c.label}</Chip>)}
    </div>
    ...
    <div style={{ display: "flex", gap: 8, marginBottom: 16 }}>
      {members.map((m, i) => (
        <button key={i} onClick={() => setF(x => ({...x, who: i}))} style={{ flex: 1, padding: "12px 8px", borderRadius: 14, border: f.who === i ? `2px solid ${m.color}` : "1.5px solid #EDE8E2", background: f.who === i ? `${m.color}08` : "white", cursor: "pointer", textAlign: "center", fontFamily: "'Noto Sans TC', sans-serif" }}>
          <div style={{ width: 28, height: 28, borderRadius: "50%", background: m.color, color: "white", fontSize: 12, fontWeight: 700, display: "inline-flex", alignItems: "center", justifyContent: "center" }}>{m.name.charAt(0)}</div>
    ...
    <div style={{ display: "flex", gap: 8, marginBottom: 24 }}>
      {[5,10,15,20].map(p => <Chip key={p} active={f.pts === p} color="#D4A843" onClick={() => setF(x => ({...x, pts: p}))}>{p} pts</Chip>)}
    </div>
      ...
      <div style={{ padding: "20px 20px 16px", background: "linear-gradient(180deg, #F5ECEC 0%, #FBF7F3 100%)", borderRadius: "0 0 28px 28px" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          ...
          <div style={{ background: "white", borderRadius: 16, padding: "8px 14px", boxShadow: "0 2px 12px rgba(0,0,0,0.04)" }}>
            ...
            <div style={{ fontFamily: "'Noto Serif TC', serif", fontSize: 18, fontWeight: 700, color: "#D4A843" }}>{totalPts} pts</div>
          </div>
        </div>
        <div style={{ display: "flex", gap: 8, marginTop: 16 }}>
          {members.map((m, i) => (
            <div key={i} style={{ flex: 1, background: "white", borderRadius: 14, padding: "10px 6px", textAlign: "center", boxShadow: "0 2px 8px rgba(0,0,0,0.03)" }}>
              <div style={{ width: 32, height: 32, borderRadius: "50%", background: m.color, color: "white", fontSize: 13, fontWeight: 700, display: "inline-flex", alignItems: "center", justifyContent: "center" }}>{m.name.charAt(0)}</div>
              <div style={{ fontSize: 11, fontWeight: 600, color: "#6B635C", marginTop: 4 }}>{m.name}</div>
              <div style={{ fontSize: 11, color: m.color, fontWeight: 500 }}>{m.streak} 天</div>
      ...
      <div style={{ padding: "16px 20px", paddingBottom: 100 }}>
        ...
        {tab === "home" && <>
          <Card>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
              ...
              {bData.length >= 2 && bData[0].count > bData[1].count * 1.8 && bData[1].count > 0 && (
                <span style={{ fontSize: 11, background: "#FFF3E0", color: "#E88B3E", padding: "4px 12px", borderRadius: 20, fontWeight: 500 }}>分配不均</span>
              )}
            </div>
            <div style={{ display: "flex", gap: 10, alignItems: "flex-end", height: 76 }}>
              {bData.map((d, i) => (
                <div key={i} style={{ flex: 1, textAlign: "center" }}>
                  ...
                  <div style={{ marginTop: 6, fontSize: 12, fontWeight: 500, color: "#6B635C" }}>{d.name}</div>
                </div>
              ))}
            </div>
          ...
          <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginBottom: 14 }}>
            ...
            {CATS.map(c => <Chip key={c.id} active={filter === c.id} color={c.color} onClick={() => setFilter(c.id)}>{c.label}</Chip>)}
          </div>
                ...
                <Card key={t.id} style={{ padding: "14px 16px", display: "flex", alignItems: "center", gap: 12 }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ display: "flex", gap: 8, marginTop: 4, flexWrap: "wrap" }}>
                      <span style={{ fontSize: 11, background: `${c?.color}11`, color: c?.color, padding: "2px 8px", borderRadius: 8, fontWeight: 500 }}>{c?.label}</span>
                      <span style={{ fontSize: 11, color: "#C4B8AE" }}>{t.due}</span>
                      <span style={{ fontSize: 11, color: "#D4A843", fontWeight: 500 }}>+{t.pts} pts</span>
                    </div>
                  </div>
                  <button onClick={() => reassign(t.id)} style={{ background: `${m?.color}11`, border: `1px solid ${m?.color}22`, borderRadius: 12, padding: "4px 10px", cursor: "pointer", fontSize: 12, display: "flex", alignItems: "center", gap: 4 }}>
                    <div style={{ width: 18, height: 18, borderRadius: "50%", background: m?.color, color: "white", fontSize: 9, fontWeight: 700, display: "flex", alignItems: "center", justifyContent: "center" }}>{m?.name?.charAt(0)}</div>
                    <span style={{ fontSize: 11, color: "#8C8278" }}>{m?.name}</span>
                  </button>
                </Card>
              );
            })}
            ...
            {done.map(t => {
              ...
              return (
                <Card key={t.id} style={{ padding: "12px 16px", display: "flex", alignItems: "center", gap: 12, opacity: 0.5, background: "#F5F0EC", marginBottom: 6 }}>
                  ...
                  <div style={{ width: 20, height: 20, borderRadius: "50%", background: m?.color, color: "white", fontSize: 9, fontWeight: 700, display: "flex", alignItems: "center", justifyContent: "center" }}>{m?.name?.charAt(0)}</div>
                </Card>
              );
            })}
          </>}
              ...
              {ms.map((m, i) => (
                <Card key={i} style={{ border: m.ok ? "1px solid #D4A84333" : undefined, opacity: m.ok ? 1 : 0.7 }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                    ...
                    {m.ok ? <span style={{ fontSize: 12, color: "#5BA86C", fontWeight: 600 }}>達成</span> : <span style={{ fontSize: 12, color: "#C4B8AE" }}>未達成</span>}
                  </div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
              {members.map((m, i) => {
                ...
                return <div key={i} style={{ textAlign: "center" }}>
                  ...
                  <div style={{ fontSize: 12, color: "#8C8278", marginTop: 2 }}>{m.name}</div>
                </div>;
              })}
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 10, marginTop: 14 }}>
            {REWARDS.map(r => {
              ...
              return <Card key={r.id} style={{ display: "flex", alignItems: "center", gap: 14 }}>
                <div style={{ width: 40, height: 40, borderRadius: 12, background: ok ? "#D4A84312" : "#F5F0EC", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                  <span style={{ fontFamily: "'Noto Serif TC', serif", fontSize: 14, fontWeight: 700, color: ok ? "#D4A843" : "#C4B8AE" }}>{r.cost}</span>
                </div>
                <div style={{ flex: 1 }}>
                  <div style={{ fontSize: 14, fontWeight: 600, color: "#3A3330" }}>{r.title}</div>
                  <div style={{ fontSize: 12, color: "#B0A498", marginTop: 2 }}>{r.desc}</div>
                </div>
                <Btn small disabled={!ok} onClick={() => redeem(r.cost)} style={{ background: ok ? "linear-gradient(135deg, #D4A843, #E88B3E)" : "#EDE8E2", boxShadow: "none" }}>兌換</Btn>
              </Card>;
            })}
          </div>
          ...
          <div style={{ marginTop: 14, display: "flex", flexDirection: "column", gap: 8 }}>
            {[...members].sort((a, b) => b.points - a.points).map((m, i) => (
              <Card key={i} style={{ display: "flex", alignItems: "center", gap: 12, border: i === 0 ? "1px solid #D4A84333" : undefined }}>
                <span style={{ fontSize: 14, fontWeight: 700, width: 28, textAlign: "center", color: i === 0 ? "#D4A843" : i === 1 ? "#B0A498" : "#C4B8AE" }}>#{i + 1}</span>
                <div style={{ width: 32, height: 32, borderRadius: "50%", background: m.color, color: "white", fontSize: 13, fontWeight: 700, display: "flex", alignItems: "center", justifyContent: "center" }}>{m.name.charAt(0)}</div>
                <div style={{ flex: 1 }}>
                  <div style={{ fontSize: 14, fontWeight: 600, color: "#3A3330" }}>{m.name}</div>
                  <div style={{ fontSize: 12, color: "#B0A498" }}>{m.streak} 天連續</div>
                </div>
      ...
      {showCTA && (
        <div style={{ position: "fixed", bottom: 68, left: "50%", transform: "translateX(-50%)", width: "calc(100% - 40px)", maxWidth: 440, zIndex: 60, animation: "sheetUp 0.4s ease" }}>
          <Card style={{ display: "flex", alignItems: "center", gap: 12, padding: "14px 16px", boxShadow: "0 8px 32px rgba(0,0,0,0.1)", border: "1px solid #E8A0BF33" }}>
            ...
            <button onClick={() => setShowCTA(false)} style={{ background: "none", border: "none", color: "#C4B8AE", cursor: "pointer", fontSize: 14 }}>&times;</button>
          </Card>
        </div>
      )}
        ...
        {navItems.map(n => (
          <button key={n.key} onClick={() => setTab(n.key)} style={{ flex: 1, border: "none", background: "none", cursor: "pointer", display: "flex", flexDirection: "column", alignItems: "center", gap: 2, fontFamily: "'Noto Sans TC', sans-serif", padding: "6px 0", color: tab === n.key ? "#D4456A" : "#C4B8AE", fontWeight: tab === n.key ? 600 : 400 }}>
            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
              {n.key === "home" && <><path d="M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"/><polyline points="9 22 9 12 15 12 15 22"/></>}
              {n.key === "miles" && <><circle cx="12" cy="8" r="7"/><polyline points="8.21 13.89 7 23 12 20 17 23 15.79 13.88"/></>}
              {n.key === "rewards" && <><polyline points="20 12 20 22 4 22 4 12"/><rect x="2" y="7" width="20" height="5"/><line x1="12" y1="22" x2="12" y2="7"/><path d="M12 7H7.5a2.5 2.5 0 0 1 0-5C11 2 12 7 12 7z"/><path d="M12 7h4.5a2.5 2.5 0 0 0 0-5C13 2 12 7 12 7z"/></>}
            </svg>
